...
public class RosConnectionSymbolCreator implements TagSymbolCreator {
   ...
   *  {
	\(topicName = (${topicName:String}, topicType = ${topicType:String}) \(, msgField = ${msgField:String}\)\?\)? }
   *
   * the pattern can be tested online at:
   * http://www.regexplanet.com/advanced/java/index.html
   */
  public static final Pattern pattern = Pattern.compile(
    "\(topicName = ((\\w+\\s*, topicType = (\\w+\\s*) \(, msgField = (\\w+\\s*\)\?\)?"
  );
  ...
  public void create(ASTTaggingUnit unit, TaggingResolver tagging) {
    if (unit.getQualifiedNameList().stream()
        ...
        .count() == 0) {
      return; // the tagging model is not conform to the RosToEmamTagSchema tagging schema
    }
  ...
  protected boolean checkScope(ASTScope scope) {
    ...
    Log.error(String.format("0xT0005 Invalid scope kind: '%s'. RosConnection expects as scope kind 'NameScope'.",
