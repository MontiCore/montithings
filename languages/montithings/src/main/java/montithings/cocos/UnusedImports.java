...
class UnusedImports implements MontiThingsASTMTCompilationUnitCoCo {
  ...
  public void check(ASTMTCompilationUnit node) {
        ...
        imports.keySet().forEach((impot) -> {
          if (!imports.get(impot)) {
            if (impot.isStar()) {
              ...
              String fqn = impot.streamImports().collect(Collectors.joining(".")) + "." + type
                  .getName(type.getNameList().size() - 1);
        ...
        imports.keySet().forEach((impot) -> {
          if (!imports.get(impot)) {
            if (impot.isStar()) {
              ...
              String fqn =
                  impot.streamImports().collect(Collectors.joining(".")) + "." + type.getName();
              imports.replace(impot, scope.resolveMany(fqn, JTypeSymbol.KIND).size() > 0);
    ...
    imports.forEach((impot, used) -> {
      if (!used) {
        Log.warn(String.format("0xMA011 The import %s is not used!",
            impot.streamImports().collect(Collectors.joining(".")) + "."),
            impot.get_SourcePositionStart());
      }
    });
