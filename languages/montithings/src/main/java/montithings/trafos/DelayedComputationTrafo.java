...
import de.monticore.expressions.commonexpressions._ast.ASTBooleanOrOpExpressionBuilder;
...
public class DelayedComputationTrafo extends BasicTransformations implements MontiThingsTrafo {
  ...
  public Collection<ASTMACompilationUnit> transform(Collection<ASTMACompilationUnit> originalModels,
    ...
    Log
      .info("Apply transformation: Delayed Computation: " + targetComp.getComponentType().getName(),
        TOOL_NAME);
    ...
    for (String parentName : TrafoUtil.findParents(originalModels, targetComp)) {
      ...
      for (ASTComponentInstantiation instantiation : instantiations) {
        for (String instanceName : instantiation.getInstancesNames()) {
          ...
          additionalTrafoModels
            .addAll(transform(targetComp, qNameInstance, origIngoingPorts, origOutgoingPorts));
  ...
  protected Collection<ASTMACompilationUnit> transform(
    ...
    addSubComponentInstantiation(compWrapper, fullyQNameDelay, delayCompName.toLowerCase(),
  ...
  protected void addBehaviorDelayComp(ASTMACompilationUnit comp, String origQNameInstance,
    ...
    // 2.1 Condition
    List<String> portsBeforeIn = origPortsIn.stream().map(p -> p.concat("_before_in"))
      .collect(Collectors.toList());
    if (!portsBeforeIn.isEmpty()) {
      ...
      ASTCallExpression storeNsInMapCallExpression = createCallExpression("storeNsInMap",
        storeNsInMapArgs.build());
      ASTExpressionStatement storeNsInMapCallExpressionStatement = MontiThingsMill
    ...
    // 3.1 Condition
    List<String> portsAfterIn = origPortsOut.stream().map(p -> p.concat("_after_in"))
      .collect(Collectors.toList());
    ...
    // set targetLatencies
    HashMap<Integer, Long> computationLatencies = dataHandler
      .getComputationLatencies(origQNameInstance);

    for (Integer index : computationLatencies.keySet()) {
      outgoingThenBlock
        .addMCBlockStatement(addSetTargetDelayIfStatement(index, computationLatencies.get(index)));
    }

    outgoingThenBlock.addMCBlockStatement(createDelayStatement());

    // 3.3 index_msg_from_comp+=1;
    ASTMCBlockStatement incIndexFromCompStatement = createIncrementVariableStatement(
      "index_msg_from_comp");
    ...
    for (String port : origPortsOut) {
      ...
      outgoingThenBlock
        .addMCBlockStatement(createAssignmentStatement(port + "_out", port + "_after_in"));
    }
  ...
  private ASTExpressionStatement createDelayStatement() {
    ...
    ASTCallExpression innerSubtractCallExpression = createCallExpression("subtract",
      innerSubtractArgs.build());
    ...
    ASTNameExpression targetDelayNameExpression = MontiThingsMill.nameExpressionBuilder()
      .setName("targetDelay").build();
    ...
    ASTCallExpression outerSubtractCallExpression = createCallExpression("subtract",
      outerSubtractArgs.build());
    ...
    ASTCallExpression delayNanosecondsExpression = createCallExpression("delayNanoseconds",
      delayNanosecondsArgs.build());
  ...
  private ASTExpression createCondition(List<String> ports) {
    ...
    for (String port : ports) {
      ASTNameExpression nameExpression = MontiThingsMill.nameExpressionBuilder().setName(port)
        .build();
  ...
  private ASTMCBlockStatement addSetTargetDelayIfStatement(int index, long delay) {
         ...
         */
    ASTNameExpression indexNameExpression = MontiThingsMill.nameExpressionBuilder()
      .setName("index_msg_from_comp").build();
