...
public class DeriveSymTypeOfSetDefinitions extends DeriveSymTypeOfSetExpressions implements
  ...
  public void traverse(ASTSetValueRange node) {
    SymTypeExpression left =
      this.acceptThisAndReturnSymTypeExpressionOrLogError(node.getLowerBound(), "0xA0311");
    SymTypeExpression right =
      this.acceptThisAndReturnSymTypeExpressionOrLogError(node.getUpperBound(), "0xA0312");
    if (node.isPresentStepsize()) {
      SymTypeExpression stepSize =
        this.acceptThisAndReturnSymTypeExpressionOrLogError(node.getStepsize(), "0xA0312");
  ...
  public void traverse(ASTListExpression node) {
    ...
    SymTypeExpression result = result = SymTypeExpressionFactory.createGenerics("List", getScope(node.getEnclosingScope()));
