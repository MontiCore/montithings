...
class TerraformCommandError(subprocess.CalledProcessError):
    def __init__(self, ret_code: int, cmd: str, out: Optional[str], err: Optional[str]):
        ...
        self.out = out
        self.err = err
        print("Error with command %s. Reason: %s", self.cmd, self.err)
...
class Terraform:
    ...
    def cmd(
        ...
        **kwargs,
        ...
        print(
            'Run command "' + " ".join(cmds) + '" as subprocess with pid ' + str(p.pid)
        )
...
class VariableFiles:
    ...
    def create(self, variables: Dict[str, str]) -> str:
        with tempfile.NamedTemporaryFile(
            ...
            print("%s is created", temp.name)
            self.files.append(temp)
            print("variables wrote to tempfile: %s", variables)
