...
public class FirstNameIsAFeature implements TaggingASTTaggingCoCo {
  ...
  public void check(ASTTagging tagging) {
    //Check if the feature diagram even exists.
      if (tagging.getSymbol().getFeatureDiagramSymbol().isEmpty()) {
    		Log.error(String.format("0xTAG0001 The feature diagram '%s' does not exist.", tagging.getName()), tagging.get_SourcePositionStart());
    	}
    
    //Check each tag to see if the feature in the feature diagram exists.
    for (ASTTag tag : tagging.getTagList()) {
    	  if (tag.getSymbol().getFeatureSymbol().isEmpty()) {
    		  Log.error(String.format("0xTAG0002 The feature '%s' does not exist in the feature diagram.", tag.getFeature().getQName()), tagging.get_SourcePositionStart());
    	  }
      }
    }
  }
