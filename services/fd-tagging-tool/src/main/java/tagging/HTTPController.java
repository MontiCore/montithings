...
import de.monticore.featureconfiguration._ast.*;
...
public class HTTPController {
    ...
    private Object handleImportFD(Request request, Response response) {
        try {
            ...
            while (entriesForTaggingSearch.hasMoreElements()){
                ...
                String extension = "";
                if (entryNameParts.length > 0) {extension = entryNameParts[entryNameParts.length - 1];}
    ...
    private Object handleSetFeatureConstraint(Request request, Response response){
        ...
        } else {return RESPONSE_JSON_FAILED;}
    }
    ...
    private Object handleGetMaximalConfigs(Request request, Response response){
        ...
        if (fdSymbol.isPresent()) {
            ...
            if (configSize != 0) {
                configs = tool.findConfigurationOfSize(tool.factTool.execAllProducts(featureDiagram), configSize);
            } else {
    ...
    private Object handleValidateFeatureConfig(Request request, Response response){
        ...
        if (fdSymbol.isPresent()) {
            ...
            return tool.factTool.execIsValid(featureDiagram, builder.build()) ? RESPONSE_JSON_SUCCESS : RESPONSE_JSON_FAILED;
        } else {return "failed";}
    }
    ...
    private Object handleCompleteFeatureConfig(Request request, Response response){
        ...
        if (fdSymbol.isPresent()) {
            ...
            ASTFeatureConfiguration outputConfig = tool.factTool.execCompleteToValid(featureDiagram, builder.build());
