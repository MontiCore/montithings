 ...
 * Software components are deployed to {@link IDeployTargetProvider}s according
 * to a {@link Distribution}.
 */
public class DeploymentManager implements IDeployStatusListener {
  ...
  private IDistributionCalculator prepareDistributionCalculator(DeploymentConfiguration config)
    ...
    try {
      ...
      for (String instanceName : hardwareRequirements.keySet()) {
        String plOCLQuery = gen.generateOCLQuery(instanceName + ":" + hardwareRequirements.get(instanceName))
            .exceptionally((t) -> {
              return null;
            }).get();
        if (plOCLQuery == null) {
  ...
  public DeploymentConfiguration computeSuggestion(DeploymentConfiguration config, int suggestionIndex)
      throws DeploymentException {
    try {
      ...
      DistributionSuggestionRequest request = new DistributionSuggestionRequest(targetProvider.getClients(),
          instanceNames, suggestionIndex, 1);
