...
public:
  explicit parse_error (const std::string &msg) : m_msg{ "parse_error: " } { m_msg.append (msg); };
...
public:
  auto
  serialize (const T &data) const -> std::string final
  {
    return dataToJson (data);
  };

  auto
  deserialize (const std::string &data) const -> T final
  {
...
public:
  auto
  serialize (const CDType &data) const -> std::string final
  {
  ...
  auto
  deserialize (const std::string &data) const -> CDType final
  {
...
class ProtobufSerializer<Message<CDPayload>> : public Serializer<Message<CDPayload>>
{
  const ProtobufSerializer<CDPayload> mInner{};

public:
  auto
  serialize (const Message<CDPayload> &data) const -> std::string final
  {
  ...
  auto
  deserialize (const std::string &data) const -> Message<CDPayload> final
  {
