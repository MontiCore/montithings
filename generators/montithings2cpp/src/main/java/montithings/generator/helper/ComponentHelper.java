...
public class ComponentHelper {
  ...
  public static String printPortValuecheck(montithings._symboltable.ComponentSymbol component,
      PortSymbol port) {
    if (!port.getAstNode().isPresent()) {
      Log.warn("0xMT011 Port " + port.getName() + " has no AST Node in symbol.");
      return "";
    }
    ASTPort montiArcPort = (ASTPort) port.getAstNode().get();
    if (!(port.getAstNode().get() instanceof montithings._ast.ASTPort)) {
      Log.info("0xMT012 Port " + port.getName() + " is no MT Port. No value checks generated.",
          "printPortValuecheck");
      return "";
    }
    montithings._ast.ASTPort mtPort = (montithings._ast.ASTPort) montiArcPort;
    if (!mtPort.isPresentAllowedValues()) {
      return "";
    }
    boolean e = port.getComponent().get().getIncomingPorts().size() > 0;
    String valueContainer = port.isIncoming() ? "input" : "result";

    StringBuilder result = new StringBuilder();
    result.append("{");

    for (ASTMTPortDeclaration decl : mtPort.getMTPortDeclarationList()) {
      String portValue =
          valueContainer + ".get" + StringUtils.capitalize(decl.getName()) + " ().value ()";
      if (!component.isBatchPort(port)) {
        result.append(formatCheckPreparation(portValue));
        result.append("if (")
            .append(valueContainer)
            .append(".get")
            .append(StringUtils.capitalize(decl.getName()))
            .append(" () && !(");
      }
      else {
        portValue = "v";
        ComponentHelper helper = new ComponentHelper(component);
        result.append("std::vector<").append(helper.getRealPortCppTypeString(port))
            .append("> copy").append(StringUtils.capitalize(decl.getName())).append(" = input.get")
            .append(StringUtils.capitalize(decl.getName()))
            .append(" ();");
        result.append("for_each(copy").append(StringUtils.capitalize(decl.getName()))
            .append(".begin(), copy").append(StringUtils.capitalize(decl.getName()))
            .append(".end(), [](").append(helper.getRealPortCppTypeString(port))
            .append("& v) {").append(formatCheckPreparation("v")).append(" if (!(");
      }

      if (mtPort.getAllowedValues() instanceof ASTMTValueList) {
        ASTMTValueList list = (ASTMTValueList) mtPort.getAllowedValues();
        result.append(valueListToString(portValue, list, valueContainer));
      }
      else if (mtPort.getAllowedValues() instanceof ASTMTValueRange) {
        ASTMTValueRange range = (ASTMTValueRange) mtPort.getAllowedValues();
        result.append(valueRangeToString(portValue, range, valueContainer));
      }
      else if (mtPort.getAllowedValues() instanceof ASTMTValueFormat) {
        ASTMTValueFormat format = (ASTMTValueFormat) mtPort.getAllowedValues();
        if (component.isBatchPort(port)) {
          result.append(formatCheckToString("allowedValuesCheck.str ()", format));
        } else {
          result.append(formatCheckToString(portValue, format));
        }
      }
      result.append(")) {");
      if (decl.isPresentDefault()) {
        if (component.isBatchPort(port)) {
          result
              .append("v =")
              .append(valuationToString(decl.getDefault()))
              .append(";");
        }
        else if (valueContainer.equals("result")) {
          result
              .append(valueContainer)
              .append(".set")
              .append(StringUtils.capitalize(decl.getName()))
        ...
        else if (valueContainer.equals("input")) {
          result
              .append(valueContainer)
              .append(".add")
              .append(StringUtils.capitalize(decl.getName()))
        ...
        result.append("std::ostringstream error;");
        result.append("error << \"Violated allowed values for port \\\"")
            .append(decl.getName())
            .append("\\\". Actual value: \";");
        if (component.isBatchPort(port)) {
          result.append("error << v;");
        }
        else {
          result.append("error << ")
              .append(valueContainer)
        ...
        result.append("throw std::runtime_error(error.str ());");
      }
      result.append("}");
      if (component.isBatchPort(port)) {
        result.append("});");
        result.append("input.set")
            .append(StringUtils.capitalize(decl.getName()))
            .append("(copy")
            .append(StringUtils.capitalize(decl.getName()))
            .append(");");
  ...
  private static String formatCheckPreparation(String port) {
    return "std::ostringstream allowedValuesCheck;"
        + "allowedValuesCheck << " + port + ";";
  }

  private static String formatCheckToString(String port, ASTMTValueFormat format) {
    return "std::regex_match(" + port + ", std::regex(\"" + format.getFormat() + "\"))";
  }

  private static String valueRangeToString(String port, ASTMTValueRange range,
      String valueContainer) {
    String lowerBound = valuationToString(range.getLowerBound());
    String upperBound = valuationToString(range.getUpperBound());

    StringBuilder result = new StringBuilder();
    result.append(port).append(" >= ").append(lowerBound);
    result.append(" && ");
    if (range.isPresentStepsize()) {
      result.append("(").
          ...
          .append(lowerBound)
          .append(" % ")
          .append(valuationToString(range.getStepsize()))
          .append(" == 0")
          .append(")");
      result.append(" && ");
    }
    result.append(port).append(" <= ").append(upperBound);
    return result.toString();
  }

  private static String valueListToString(String port, ASTMTValueList list,
      String valueContainer) {
    List<ASTMTValueRange> ranges = list.getMTValueRangeList();
    List<ASTValuation> valuations = list.getValuationList();

    StringBuilder result = new StringBuilder();
    for (ASTValuation v : valuations) {
      result.append(port).append(" == ").append(valuationToString(v));
      result.append(" || ");
    }
    for (ASTMTValueRange r : ranges) {
      result.append("( ").append(valueRangeToString(port, r, valueContainer)).append(" )");
      result.append(" || ");
    }
    // remove last " && "
    result.setLength(Math.max(result.length() - 4, 0));
    return result.toString();
  }

  private static String valuationToString(ASTValuation valuation) {
    return printExpression(valuation.getExpression());
  }

  private static String printExpression(ASTExpression expr, boolean isAssignment) {
