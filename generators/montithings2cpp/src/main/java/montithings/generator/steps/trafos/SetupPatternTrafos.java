    ...
    public void action(GeneratorToolState state) {
        ...
        if (config.getApplyPatterns() == ApplyPatterns.ON) {
            if (state.shouldCreateUnivariateAnomalyDetection() || state.shouldCreateMultivariateAnomalyDetection()) {
                tool.addTrafo(
                        new SetupAnomalyDetectionPatternTrafo(
                                state.getModelPath(),
                                WINDOW_SIZE,
                                TOLERANCE,
                                Paths.get(state.getTarget().getAbsolutePath(), "hwc").toFile(),
                                state.getTarget(),
                                state.shouldCreateUnivariateAnomalyDetection(),
                                state.shouldCreateMultivariateAnomalyDetection()
                        )
                );
            }

            tool.addTrafo(new AnomalyDetectionPatternTrafo(state.getModelPath()));
        }
    }
}
