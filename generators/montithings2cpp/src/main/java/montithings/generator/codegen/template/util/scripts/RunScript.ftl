...
<#list instances as pair >
  <#if brokerIsMQTT && ComponentHelper.hasHandwrittenPythonBehaviour(config.hwcPath, pair.key) && !ComponentHelper.isDSLComponent(pair.key,config)>
...
<#list instances as pair >
  <#if !ComponentHelper.hasHandwrittenPythonBehaviour(config.hwcPath, pair.key) || ComponentHelper.isDSLComponent(pair.key,config)>
    <#if brokerIsMQTT>
