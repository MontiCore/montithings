...
<#list comp.getPorts() as p>
    <#if p.isOutgoing()>
        ...
        comp->getInterface()->addOutPort${p.getName()?cap_first}(new DDSPort<${type}>(*this, OUTGOING, comp->getInstanceName() + ".${p.getName()}/out", "${p.getName()}", true, false));
            ...
            <#list comp.getAstNode().getConnectors() as connector>
                <#list connector.getTargetList() as target>
                    <#if target.getQName() == p.getName()>
                        ...
                        comp->getInterface()->addInPort${p.getName()?cap_first}(new DDSPort<${type}>(*this, INCOMING, topic, "${p.getName()}", true, false));
                    </#if>
                </#list>
            </#list>
        </#if>
