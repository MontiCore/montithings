...
public class ExpectedErrorInfo {
	...
	/**
	 * Raises an error if the given error codes don't match the convention for error
	 * codes in test cases (no leading zero, capital hexadecimal digits)
	 */
		...
		for (String errorCode : errorCodes) {
			if (!ERROR_CODE_PATTERN.matcher(errorCode).matches()) {
				Log.error(String.format(
						"The given expected error code \"%s\" is not a valid error code (pattern: \"%s\")", errorCode,
						ERROR_CODE_PATTERN.pattern()));
			}
		}
	}
	...
	private String concatenateFindings(List<Finding> findings) {
		return findings.stream().map(f -> f.buildMsg()).collect(Collectors.joining("\n"));
	}
