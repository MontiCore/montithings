...
public abstract class AbstractCoCoTest {
	...
	/**
	 * Loads the component AST Node for the specified model from the specified
	 * package.
	 * 
	...
	/**
	 * Load the component with the given name from the given package that resides in
	 ...
	 */
	protected ComponentSymbol loadComponentSymbolFromModelPath(String qualifiedModelName, String modelPath) {
		Scope symTab = MONTIARCTOOL.initSymbolTable(Paths.get(modelPath).toFile(),
				Paths.get(FAKE_JAVA_TYPES_PATH).toFile());
		ComponentSymbol comp = symTab.<ComponentSymbol>resolve(qualifiedModelName, ComponentSymbol.KIND).orElse(null);
		return comp;
	}
	 ...
	 * @return The loaded component symbol
	 */
	protected ComponentSymbol loadComponentSymbolFromModelPath(String packageName, String modelName, String modelPath) {
		return loadComponentSymbolFromModelPath(packageName + "." + modelName, modelPath);
	}
	...
	/**
	 * Initializes the symbol table with the normal model path and the types of the
	 * java standard library, like java.util, java.lang, etc
	 * 
	 * @return The symbol table
	 */
	protected Scope loadDefaultSymbolTable() {
		return MONTIARCTOOL.initSymbolTable(Paths.get(MODEL_PATH).toFile(), Paths.get(FAKE_JAVA_TYPES_PATH).toFile());
	}
	...
	protected static void checkInvalid(MontiArcCoCoChecker cocos, ASTMontiArcNode node,
		...
		expectedErrors.checkOnlyExpectedPresent(Log.getFindings(),
				"Got no findings when checking only " + "the given coco. Did you pass an empty coco checker?");
	}
