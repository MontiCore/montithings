...
After the call `setNextValue` on the port _value_, the _PortSpy_ objects contain a list of all messages which passes the ports. This list is used for checks if ports gets a given message. [The asserts](#5-the-asserts) gives a detailed explanation of how the values of the _PortSpy_ objects will check.
